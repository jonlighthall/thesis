...
The print edition was issued by ProQuest/UMI Dissertation Publishing under Order No.~3471048 (ISBN~978-1-249-83864-7). The author's manuscript is 170 pages; the published edition runs slightly longer because ProQuest inserts its own publishing page (and, in some bindings, a blank verso) ahead of the title page, which accounts for the 2--3 page difference between copies. The official ProQuest download is 172 pages.

\subsection{Digital Editions}
\subsubsection*{ProQuest Dissertations \& Theses Global}
The official publisher record is available through ProQuest. The stable URL provided by ProQuest (suffix \texttt{/se-2}) is the recommended persistent link:
\begin{quote}\sloppy
\url{https://www.proquest.com/dissertations-theses/commissioning-helical-orbit-spectrometer-new/docview/885145848/se-2}
\end{quote}
A shortened form using only the ProQuest Document ID also resolves to the same record:
\begin{quote}
...
The author is the copyright holder of the original published work, U.S.\ Copyright Office Registration Number~\textbf{TX0006735169}, effective October~31,~2011. The registration number is itself the canonical, persistent identifier for the record; the U.S.\ Copyright Office does not assign DOIs. The corresponding entry in the Copyright Public Records System can be retrieved at:
\begin{quote}\sloppy
...
\section{Citations}
The published work has been cited by the following publications (both doctoral dissertations):
\begin{enumerate}
    \item ``Development of Novel Scintillation Detection Techniques for Use in Nuclear Physics and Medical Applications.''
    \item ``A direct study of $^{20}$Ne($\alpha$,$p$)$^{23}$Na with the HELIcal Orbit Spectrometer (HELIOS).'' \href{https://doi.org/10.31390/gradschool_dissertations.2625}{doi:10.31390/gradschool\_dissertations.2625}
\end{enumerate}